    ...
    <div className="rounded-xl overflow-hidden" style={{ background: '#242428', border: '0.5px solid rgba(255,255,255,0.07)' }}>
      ...
      {!collapsed && (
        <>
          ...
          {itemIds.length === 0 && (
            <div
              className="flex items-center gap-2.5 px-3.5 py-5 cursor-pointer"
...
export default function PackageBuilderPage() {
  ...
  const isInPackage = useCallback((id: string) => allPackageIds.includes(id), [allPackageIds])
  ...
  const removeItem = useCallback((id: string, cat: GearCategory) => {
    setPackageItems(prev => ({ ...prev, [cat]: prev[cat].filter(i => i !== id) }))
  }, [])

  const selectItem = useCallback((id: string) => setActiveItemId(id), [])
  const browseCategory = useCallback((cat: GearCategory) => setActiveCategory(cat), [])

  return (
    <div className="flex flex-col h-screen" style={{ background: '#18181A', color: '#EAEAEA' }}>

      {/* Top bar */}
      <header className="flex items-center gap-3 px-4 h-[52px] flex-shrink-0" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
        <div>
          <p className="text-[13px] font-medium">Meridian — Feature Package</p>
        ...
        <select
          ...
          style={{ background: '#242428', border: '0.5px solid rgba(255,255,255,0.08)', color: '#9090A0' }}
        >
        ...
        <div className="flex items-center gap-2 rounded-md px-3 py-1.5" style={{ background: '#242428', border: '0.5px solid rgba(255,255,255,0.08)' }}>
          <span className="text-[11px]" style={{ color: '#555560' }}>Budget</span>
          <div className="h-1 rounded-full" style={{ background: '#2E2E34', width: 72 }}>
            <div className="h-full rounded-full transition-all" style={{ width: `${budgetPct}%`, background: budgetPct > 80 ? '#F0BA4A' : '#4ABA82' }} />
          </div>
          <span className="text-xs font-mono font-medium">${fmt(totalDayRate)}</span>
        </div>
        <div className="w-px h-4" style={{ background: 'rgba(255,255,255,0.1)' }} />
        <button className="rounded-lg px-3.5 py-1.5 text-xs font-medium" style={{ background: '#FFCF7B', color: '#2A1F00' }}>
          Send quote ↗
        </button>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Left: Catalog */}
        <aside className="flex flex-col overflow-hidden flex-shrink-0" style={{ width: 240, borderRight: '0.5px solid rgba(255,255,255,0.07)' }}>
          <div className="p-2.5" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
            <div className="relative">
              <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="#555560" strokeWidth="1.8" strokeLinecap="round" className="absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none">
                <circle cx="7" cy="7" r="4.5" /><path d="M10.5 10.5l2.5 2.5" />
              </svg>
              <input
                type="text" placeholder="Search gear…" value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full rounded-lg pl-8 pr-3 py-1.5 text-xs outline-none"
                style={{ background: '#242428', border: '0.5px solid rgba(255,255,255,0.08)', color: '#EAEAEA' }}
              />
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {(['camera', 'lenses', 'support', 'focus', 'video'] as GearCategory[]).map(cat => (
            <PackageSection
              key={cat} title={CATEGORY_LABELS[cat]} category={cat}
              itemIds={packageItems[cat]} activeItemId={activeItemId}
              onSelectItem={selectItem} onRemoveItem={removeItem} onBrowseCategory={browseCategory}
            />
          ))}
        </main>

        {/* Right: Context */}
        <aside className="flex flex-col overflow-hidden flex-shrink-0" style={{ width: 220, borderLeft: '0.5px solid rgba(255,255,255,0.07)', background: '#1C1C1F' }}>
          <div className="flex" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
            {(['detail', 'budget'] as ContextTab[]).map(tab => (
              <button
                key={tab} onClick={() => setContextTab(tab)}
                className="flex-1 py-2.5 text-[11px] font-medium capitalize transition-colors"
                style={{ color: contextTab === tab ? '#EAEAEA' : '#555560', borderBottom: contextTab === tab ? '1.5px solid #3D55A8' : '1.5px solid transparent' }}
              >{tab}</button>
            ))}
          </div>
          <div className="flex-1 overflow-y-auto p-3">
            ...
            {contextTab === 'detail' && activeItem && (
              <>
                <div className="w-full h-24 rounded-lg mb-2.5 flex items-center justify-center" style={{ background: '#2E2E34' }}>
                  <span className="text-[11px] font-mono" style={{ color: '#555560' }}>{activeItem.category.toUpperCase()}</span>
                </div>
                <p className="text-[13px] font-medium leading-snug mb-0.5">{activeItem.name}</p>
                ...
                <div className="flex flex-col mb-3">
                  {activeItem.specs.map(([key, val]) => (
                    <div key={key} className="flex justify-between py-1.5" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <span className="text-[11px] text-right" style={{ color: val==='In stock'?'#4ABA82':val==='Limited'?'#F0BA4A':'#BDBDC8', maxWidth:110 }}>{val}</span>
                    </div>
                  ))}
                </div>
            ...
            {contextTab === 'budget' && (
              <>
                <p className="text-[11px] font-medium mb-2">Daily breakdown</p>
                {(['camera','lenses','support','focus','video'] as GearCategory[]).map(cat => {
                  ...
                  return (
                    <div key={cat} className="flex justify-between py-1">
                      ...
                      <span className="text-[11px] font-mono" style={{ color: total>0?'#9090A0':'#444450' }}>{total>0?`$${fmt(total)}`:'—'}</span>
                    </div>
                  )
                })}
                <div className="flex justify-between py-2 mt-1" style={{ borderTop: '0.5px solid rgba(255,255,255,0.08)' }}>
                  <span className="text-[11px] font-medium">Total / day</span>
                  <span className="text-[13px] font-medium font-mono">${fmt(totalDayRate)}</span>
                </div>
                <div className="mt-3 pt-3" style={{ borderTop: '0.5px solid rgba(255,255,255,0.06)' }}>
                  <p className="text-[11px] font-medium mb-2">{SHOOT_DAYS}-day estimate</p>
                  <div className="flex justify-between py-1"><span className="text-[11px]" style={{color:'#666672'}}>At day rate</span><span className="text-[11px] font-mono" style={{color:'#9090A0'}}>${fmt(totalEstimate)}</span></div>
                  <div className="flex justify-between py-1"><span className="text-[11px]" style={{color:'#666672'}}>Approved budget</span><span className="text-[11px] font-mono" style={{color:'#4ABA82'}}>${fmt(APPROVED_BUDGET_TOTAL)}</span></div>
                  <div className="flex justify-between py-1"><span className="text-[11px]" style={{color:'#666672'}}>Remaining</span><span className="text-[11px] font-mono" style={{color:remaining>=0?'#4ABA82':'#E06B6B'}}>{remaining>=0?`$${fmt(remaining)}`:`-$${fmt(Math.abs(remaining))}`}</span></div>
                </div>
                <div className="mt-3 pt-3" style={{ borderTop: '0.5px solid rgba(255,255,255,0.06)' }}>
