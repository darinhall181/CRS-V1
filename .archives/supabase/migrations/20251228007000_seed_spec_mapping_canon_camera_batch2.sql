-- Seed migration (idempotent): Canon camera mapping batch 2 (UI-top specs)
-- Focus: Wi-Fi/Bluetooth details, LCD Screen attributes, and a few high-frequency Canon labels.
...
BEGIN
  ...
  INSERT INTO spec_definition (section_id, display_name, normalized_key, data_type, unit, category_id, importance) VALUES
    -- Connectivity: split Wi‑Fi vs Bluetooth so we don't jam everything into generic `wireless`
    ...
    -- Display: Canon LCD Screen emits many stable labels
  ...
  FROM (
    VALUES
      -- Wi‑Fi / Bluetooth sections (Canon uses section names like "Wi‑Fi®" / "Bluetooth®")
      ...
      -- LCD Screen / LCD Monitor section labels
      ...
      -- Ports (map Canon "Headphone terminal" to a text field, not boolean headphone_port)
